...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data) {
  ...
  return i2c_master_transmit(es->i2c_dev_handle, write_buf, sizeof(write_buf), pdMS_TO_TICKS(1000));
...
static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value) {
  ...
  return i2c_master_transmit_receive(es->i2c_dev_handle, &reg_addr, 1, reg_value, 1, pdMS_TO_TICKS(1000));
}
...
static esp_err_t es8311_fmt_config(es8311_handle_t dev, const es8311_resolution_t res_in, const es8311_resolution_t res_out) {
  ...
  reg00 &= 0xBF;
  ESP_RETURN_ON_ERROR(
      es8311_write_reg(dev, ES8311_RESET_REG00, reg00), TAG, "I2C read/write error");  // Slave serial port - default
...
esp_err_t es8311_init(es8311_handle_t dev,
  ...
  if (!clk_cfg->mclk_from_mclk_pin) {
    ESP_RETURN_ON_FALSE(
        res_out == res_in, ESP_ERR_INVALID_ARG, TAG, "Resolution IN/OUT must be equal if MCLK is taken from SCK pin");
  }
  ...
  ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_SYSTEM_REG0E, 0x02),
                      TAG,
                      "I2C read/write error");  // Enable analog PGA, enable ADC modulator - NOT default
  ESP_RETURN_ON_ERROR(
      es8311_write_reg(dev, ES8311_SYSTEM_REG12, 0x00), TAG, "I2C read/write error");  // power-up DAC - NOT default
  ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_SYSTEM_REG13, 0x10),
                      TAG,
                      "I2C read/write error");  // Enable output to HP drive - NOT default
  ESP_RETURN_ON_ERROR(es8311_write_reg(dev, ES8311_ADC_REG1C, 0x6A),
                      TAG,
                      "I2C read/write error");  // ADC Equalizer bypass, cancel DC offset in digital domain
  ESP_RETURN_ON_ERROR(
      es8311_write_reg(dev, ES8311_DAC_REG37, 0x08), TAG, "I2C read/write error");  // Bypass DAC equalizer - NOT default
...
es8311_handle_t es8311_create(const i2c_master_bus_handle_t i2c_bus_handle, const uint16_t dev_addr) {
  es8311_dev_t *sensor = (es8311_dev_t *)calloc(1, sizeof(es8311_dev_t));
  sensor->dev_addr = dev_addr;
  ...
  esp_err_t ret = i2c_master_bus_add_device(i2c_bus_handle, &dev_config, &sensor->i2c_dev_handle);
  printf("i2c_master_bus_add_device ret = %d\n", ret);
  printf("sensor->i2c_dev_handle:%p\n", sensor->i2c_dev_handle);
